...
//  mkugraph
//
// create random undirected graph upto 64 nodes and 2016 edges
...
int main(int argc, const char * argv[]) {
    int nn, ne, i, j, k, p[128], last, ek, tmp;
    int cc[2048], ck, ncc, cmap[2048], mk, f, fi, fj;
    FILE* gfile;
    std::tuple<int,int> e;
    std::unordered_set <std::tuple<int,int>, hashtuple> ee;
    time_t tim;
    
    if (argc<4) { printf("mkugraph <filename> <numnodes> <numedges>\n");  exit(0); }
    time(&tim);
    srandom((unsigned int) tim);
    nn = atoi(argv[2]);  ne = atoi(argv[3]);
    printf("numnodes=%d    numedges=%d\n", nn, ne);
    
    if (ne > nn*(nn-1)/2) { printf("ERROR. ne exceeds nn*(nn-1)/2\n");  exit(0); }
    ...
    mk = last-1;
    ck = cmap[mk];
    if (cc[ck] != last)  printf("PROBLEM\n");
    f=cc[--ncc];            // filler is last in list
    cc[ck] = f;             // hole-fill.  filler has moved so update map
    fi = f>>16;  fj=f & 0xffff;
    mk = (nn-1)*fi +fj - fi*(fi+1)/2 -1;
    cmap[mk]=ck;
